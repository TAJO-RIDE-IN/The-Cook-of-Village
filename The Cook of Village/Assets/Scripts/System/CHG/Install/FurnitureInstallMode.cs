...
public class FurnitureInstallMode : InstallMode
{
    ...
    {
        for (int i = 11; i < 13; i++)//?òÏûêÎß??πÏàò??UI ?ÑÏ?
        {
    ...
    {
        for (int i = 0; i < 2; i++)//?òÏûêÎß??πÏàò??UI ?ÑÏ?
        {
            if (name == _furniturePooling.poolObjectData[i].name)
            {
                return true;
    ...
    {
        for (int i = 2; i < 10; i++)//?òÏûêÎß??πÏàò??UI ?ÑÏ?
        {
    ...
    private void CheckChairPosition()
    {
        ...
        for (int j = 0; j < tableChairs.Count; j++)
        {
            ...
            {
                secondDis = Vector3.Distance(pendingObject.transform.position, tableChairs[j].tablePos);//?åÏù¥Î∏îÍ≥º??Í±∞Î¶¨Ï∞®Ïù¥
                //Debug.Log(secondDis);
