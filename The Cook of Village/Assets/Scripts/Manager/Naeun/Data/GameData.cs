...
public class GameData : DataManager<GameData>, IGameDataOb
{
    ...
    public void LoadObject()
    {
        ...
        if (runningCoroutine != null) { StopCoroutine(runningCoroutine); }//??Í∞úÏùò ÏΩîÎ£®?¥Îßå ?§Ìñâ
        if (GuestCount > 0)
        {
    ...
    public float TimeOfDay //24?úÍ∞Ñ => 1440Î∂?
    { 
    ...
    /// <summary>
    /// ?∞Ïù¥?∞Î? ?Ä?•Ìï† ???àÎäîÏßÄ ?ïÏù∏
    /// </summary>
    /// <returns>Î∂ÑÏù¥ 0?¥Í≥† ?Ä?•ÏùÑ ?????ÅÌÉú????true Î¶¨ÌÑ¥</returns>
    ...
    {
        ForciblySleep = false;
        SaveDataTime(PlayerName);//?êÎã§ ?ºÏñ¥?¨ÏùÑ???Ä??
    ...
    #endregion
    #endregion
    public void ResetData() //?úÏûë ?îÎ©¥?ºÎ°ú ?åÏïÑ?îÏùÑ???∞Ïù¥??Î¶¨ÏÖã
    {
    ...
    public void NotifyObserver(List <IObserver<GameData>> observer) //observer??Í∞??ÑÎã¨
    {
